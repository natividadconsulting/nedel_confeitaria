...
export default function CheckoutPage() {
    ...
    return (
      <div className="min-h-screen bg-[#FDF8F0] flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-sm w-full">
          ...
          <h2 className="text-2xl font-bold text-stone-800 mb-2">Quase lá!</h2>
          <p className="text-stone-600 mb-6">
            {isPixPayment
              ? 'Pague o PIX e depois envie seu pedido pelo WhatsApp para finalizar.'
              : 'Envie seu pedido pelo WhatsApp para que a Confeitaria Nedel possa prepará-lo.'}
          </p>

          {isPixPayment && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-4 text-left">
              <p className="text-xs font-bold text-green-700 uppercase tracking-wide mb-2">Passo 1 — Pague o PIX</p>
              <p className="text-sm text-green-800 mb-1">Chave PIX (Telefone)</p>
              <p className="font-mono text-xl font-bold tracking-wide text-green-900">{PIX_KEY}</p>
              <p className="text-xs text-green-600 mt-2">Envie o comprovante junto com seu pedido no WhatsApp.</p>
            </div>
          )}

          <div className="bg-red-50 border-2 border-red-300 rounded-xl p-3 mb-4 text-left">
            <p className="text-sm font-bold text-red-700">⚠️ Não feche este app ainda!</p>
            <p className="text-xs text-red-600 mt-0.5">
              {isPixPayment
                ? 'Após pagar, volte aqui e clique no botão abaixo. Sem isso, o pedido não chega à loja.'
                : 'Clique no botão abaixo para enviar seu pedido pelo WhatsApp. Sem isso, o pedido não chega à loja.'}
            </p>
          </div>
          ...
          >
            ...
            {isPixPayment ? 'Passo 2 — Enviar pelo WhatsApp' : 'Enviar Pedido pelo WhatsApp'}
          </a>
  ...
  const isValid = form.name.trim() && form.phone.trim() && (form.deliveryType === 'retirada' || form.address?.trim()) && form.preferredDate && form.preferredTime
    ...
    <div className="min-h-screen bg-[#FDF8F0]">
        ...
        <div className="bg-white rounded-2xl border border-stone-100 overflow-hidden">
          ...
          {form.deliveryType === 'entrega' && (
            <div className="px-4 py-2 border-t border-stone-100 flex justify-between items-center">
              <span className="text-xs text-stone-500">🚚 Taxa de entrega</span>
              <span className="text-xs font-medium text-stone-500">a combinar via WhatsApp</span>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-4 space-y-3">
          ...
          {form.deliveryType === 'entrega' && (
            <div>
              ...
              />
              <p className="text-xs text-amber-700 mt-1">A taxa de entrega será informada pela loja pelo WhatsApp após o recebimento do pedido.</p>
            </div>
          )}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#FDF8F0] border-t border-amber-100">
        <div className="max-w-xl mx-auto">
          ...
          {!isValid && (
            <p className="text-xs text-center text-stone-400 mt-2">Preencha nome, telefone, data e horário para continuar</p>
          )}
        </div>
      </div>
    </div>
