...
All category descriptions are purely descriptive — no prices. Prices are visible on individual product cards.

### Pelotine Label
The modal uses just "Pelotine" — "(coperinha)" clarification was removed as unnecessary.

## Menu Data Confirmations (from image scan)

### Stikadinho com Morango — Tortas Especiais
Confirmed via product photos in the catalog: "Stikadinho com Morango — leite ninho com morango e stikadinho" belongs in Tortas Especiais.
...
### Risoles Pricing
Risoles de Frango, Carne, and Presunto e Queijo all use the same pricing as the rest of Salgados Tradicionais (R$1,35/un · R$67,50/50un · R$120/100un). A separate RISOLES_BULK at R$1,25 was incorrect and has been removed.
...
### Subcategories
Four subcategories: Mini Pizzas, Kibes, Empadinhas Francesas, Especialidades. The Especialidades section (Mini Cachorro Quente, Mini Hambúrguer) is per-unit only with no bulk pricing.

### Pelotine
Mini Pizzas, Kibes, and Empadinhas Francesas are **com pelotine only** (`pelotine: 'com-only'`). All three quantity options (unidade, 50un, 100un) show "com pelotine" on the card — no sem/com toggle.
